...
-- Q1:Calculate total revenue for each game: Total Revenue = Number_of_Purchases * Unit_Price
select game_id,
sum(Number_of_Purchases * Unit_Price) as total_revenue from games_revenue
group by game_id;

-- Q2:What are the top 5 games by revenue?
select game_id,
sum(Number_of_Purchases * Unit_Price) as total_revenue from games_revenue
group by game_id
...
-- Q3:Identify the top 3 genres by total revenue.
...
-- Q4:Which year generated most of the revenue in the company.
...
-- Q5:Calculate the percentage of customers who left reviews out of the total purchase.
...
-- Q6:Determine the percentage of reviews written in English for each game
...
-- Q7:Find the top 5 publishers by total revenue.
...
-- Q8:Calculate the percentage of Helpful and Funny tags to total reviews. 
...
-- RANK by reviews in each genre(Use RANK() to rank games by their total reviews within each genre.)
SELECT* 
FROM games_reviews;
